...
class AStarPlanner:
    ...
    class Node:
        def __init__(self, x, y, yaw, f, g, parent_index):
            self.x = x  # index of grid
            self.y = y  # index of grid
            self.yaw = yaw  # angle [rad]
            self.f = f
            self.g = g
            self.parent_index = parent_index

        def __str__(self):
            return str(self.x) + "," + str(self.y) + "," + str(self.yaw) + "," + str(self.f) + "," + str(
                self.g) + "," + str(self.parent_index)
    ...
    def planning(self, sx, sy, syaw, gx, gy, gyaw):
        ...
        start_node = self.Node(self.calc_xy_index(sx, self.min_x),
                               ...
                               self.calc_yaw_index(syaw), 0.0, 0.0, -1)
        goal_node = self.Node(self.calc_xy_index(gx, self.min_x),
                              self.calc_xy_index(gy, self.min_y),
                              self.calc_yaw_index(gyaw), 0.0, 0.0, -1)
        ...
        while 1:
            ...
            if current.x == goal_node.x and current.y == goal_node.y and current.yaw == goal_node.yaw:
                ...
                goal_node.g = current.g
                break

            # Removing the item from the open set
            del open_set[c_id]

            # Adding it to the closed set
            ...
            for i, _ in enumerate(self.motion):
                node = self.Node(current.x + self.motion[i][0],
                                 ...
                                 (current.yaw + self.motion[i][2]) % self.calc_yaw_index(math.pi * 2),
                                 0.0, 0.0, c_id)
    ...
    def calc_final_path(self, goal_node, closed_set):
        ...
        rx, ry, ryaw = [self.calc_grid_position(goal_node.x, self.min_x)], [
            self.calc_grid_position(goal_node.y, self.min_y)], [
                            self.calc_yaw_angle(goal_node.yaw)]
        parent_index = goal_node.parent_index
        while parent_index != -1:
            n = closed_set[parent_index]
            rx.append(self.calc_grid_position(n.x, self.min_x))
            ry.append(self.calc_grid_position(n.y, self.min_y))
            ryaw.append(self.calc_yaw_angle(n.yaw))
            parent_index = n.parent_index

        return rx, ry, ryaw

    def calc_yaw_angle(self, yaw):
        return yaw * (2 * math.pi / self.calc_yaw_index(math.pi * 2))

    @staticmethod
    def calc_heuristic(n1, n2):
        # taking weight of heuristic as 1
        
        return  1 * math.hypot(n1.x - n2.x, n1.y - n2.y)
    ...
    def calc_yaw_index(self, yaw):
        ...
        :return:
        """
        resolution_yaw = math.radians(30)
        return round(yaw / resolution_yaw)

    def calc_grid_index(self, node):
        return (node.yaw - self.min_y) * self.x_width * self.y_width + \
